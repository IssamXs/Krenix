...
export default function SmsSettingsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) { setError(d.error ?? t('smsSettings.genericError')); return }
      setConnected(true); setSid(''); setToken('')
    } finally { setSaving(false) }
  }

  const remove = async () => {
    if (!confirm(t('smsSettings.confirmDisconnect'))) return
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="dash-font-heading font-medium text-[28px] text-dash-ink">{t('smsSettings.title')}</h1>
        <p className="text-dash-ink-soft text-sm mt-1">{t('smsSettings.subtitle')}</p>
      </div>

      {locked ? (
        <LockedFeatureCard title={t('smsSettings.lockedTitle')} requiredPlan="Business" />
      ) : (
        <Card className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <MessageSquare size={16} className="text-dash-accent" />
              <h3 className="text-dash-ink font-bold text-sm">{t('smsSettings.twilioAccount')}</h3>
            </div>
            {connected && (
              <span className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-lg bg-dash-success-soft text-dash-success">
                <Check size={12} /> {t('smsSettings.connected', { sender })}
              </span>
            )}
          </div>
          ...
          <div>
            <label className={LABEL}>{t('smsSettings.accountSid')}</label>
            <input value={sid} onChange={e => { setSid(e.target.value); setError('') }} placeholder="ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" className={INPUT} />
          </div>
          <div>
            <label className={LABEL}>{t('smsSettings.authToken')}</label>
            <input value={token} onChange={e => { setToken(e.target.value); setError('') }} type="password" placeholder={t('smsSettings.authTokenPlaceholder')} className={INPUT} />
          </div>
          <div>
            <label className={LABEL}>{t('smsSettings.senderNumber')}</label>
          ...
          <div className="flex items-center gap-2">
            <button onClick={save} disabled={saving || !sid.trim() || !token.trim() || !sender.trim()}
              ...
              {saving ? <Loader2 size={14} className="animate-spin" /> : t('smsSettings.verifyConnect')}
            </button>
            {connected && (
              <button onClick={remove} className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-xs text-dash-danger/70 hover:text-dash-danger border border-dash-border hover:border-dash-danger/30 transition-all">
                <Trash2 size={13} /> {t('smsSettings.disconnect')}
              </button>
            )}
          </div>
          <p className="text-dash-ink-faint text-[11px]">
            {t('smsSettings.footnote')}
          </p>
        </Card>
      )}
    </div>
