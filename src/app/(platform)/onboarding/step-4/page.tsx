    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-73px)] px-4 py-12">
      ...
      <div className="flex items-center gap-2 mb-12">
        {[1, 2, 3, 4].map((step) => (
          <div key={step} className="flex items-center gap-2">
            ...
            {step < 4 && <div className={`w-8 h-px ${step < 4 ? 'bg-dash-accent/30' : 'bg-dash-border'}`} />}
          </div>
        ))}
      </div>
      ...
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          <h1 className="dash-font-heading text-2xl font-medium text-dash-ink mb-2">Votre premier produit</h1>
          <p className="text-dash-ink-soft text-sm">Vous pourrez en ajouter d'autres depuis le tableau de bord</p>
        </div>

        <div className="bg-dash-surface border border-dash-border rounded-[24px] p-6 space-y-4 shadow-[0_24px_60px_-24px_rgba(20,26,33,0.18)]">
          {error && (
          ...
          {/* Image upload */}
          <div>
            <label className="block text-xs font-medium text-dash-ink-soft mb-2 uppercase tracking-wider">Photo du produit</label>
            <label className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-dash-border hover:border-dash-accent/40 cursor-pointer transition-all">
              {imageUrl ? (
                ...
                <div className="w-14 h-14 rounded-lg bg-dash-surface-2 flex items-center justify-center flex-shrink-0">
                  {uploading ? <Loader2 size={20} className="animate-spin text-dash-ink-faint" /> : <Upload size={20} className="text-dash-ink-faint" />}
                </div>
              )}
              <div>
                <p className="text-dash-ink text-sm">{imageUrl ? 'Changer la photo' : 'Ajouter une photo'}</p>
                <p className="text-dash-ink-faint text-xs mt-0.5">PNG, JPG — max 5MB</p>
              </div>
          ...
          <div>
            <label className="block text-xs font-medium text-dash-ink-soft mb-2 uppercase tracking-wider">Nom du produit *</label>
            <input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-dash-ink-soft mb-2 uppercase tracking-wider">Prix (DZD) *</label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-dash-ink-soft mb-2 uppercase tracking-wider">Stock</label>
              <input
          ...
          <div>
            <label className="block text-xs font-medium text-dash-ink-soft mb-2 uppercase tracking-wider">Couleurs (séparées par virgule)</label>
            <input
          ...
          <div>
            <label className="block text-xs font-medium text-dash-ink-soft mb-2 uppercase tracking-wider">Description</label>
            <textarea
          ...
          <button onClick={handleSkip} disabled={saving} className="w-full text-xs text-dash-ink-faint hover:text-dash-ink-soft transition-colors py-1">
            Passer cette étape
          </button>
        </div>
      </div>
