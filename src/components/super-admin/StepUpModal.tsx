  ...
  const modal = pending ? (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-dash-surface border border-dash-border rounded-[20px] p-6 w-full max-w-sm space-y-4">
        <div className="flex items-center gap-2"><ShieldAlert size={18} className="text-dash-gold-dark" /><h3 className="text-dash-ink font-bold">Confirmer votre identité</h3></div>
        <p className="text-dash-ink-soft text-xs">Action sensible — entrez votre mot de passe pour continuer.</p>
        ...
        <input type="password" value={password} autoFocus onChange={e => setPassword(e.target.value)}
          ...
          className="w-full px-4 py-3 rounded-xl bg-dash-surface-2 border border-dash-border text-dash-ink placeholder-dash-ink-faint outline-none focus:border-dash-accent/50 text-sm" />
        <div className="flex gap-3">
          <button onClick={() => { setPending(null); setPassword('') }} className="flex-1 py-3 rounded-xl border border-dash-border text-dash-ink-soft hover:text-dash-ink text-sm">Annuler</button>
          <button onClick={submit} disabled={busy || !password} className="flex-1 flex items-center justify-center py-3 rounded-xl bg-dash-gold hover:bg-dash-gold-dark text-white font-semibold text-sm disabled:opacity-50">
