...
import { TECH_TOKENS } from './techDefaults'
...
export default function TechLanding({ landingPage, store }: Props) {
  const [openSection, setOpenSection] = useState<number | null>(0)
  const [showModal, setShowModal] = useState(false)

  const theme = store.theme?.config
  const bg = theme?.colors.background ?? TECH_TOKENS.bg
  const card = theme?.colors.card ?? TECH_TOKENS.card
  const primary = theme?.colors.primary ?? TECH_TOKENS.primary
  const text = theme?.colors.text ?? TECH_TOKENS.text
  const textMuted = theme?.colors.textMuted ?? TECH_TOKENS.textMuted
  const border = theme?.colors.border ?? TECH_TOKENS.border
  ...
  const headingFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.heading ?? TECH_TOKENS.heading}', sans-serif`
  const bodyFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.body ?? TECH_TOKENS.body}', sans-serif`
  ...
  return (
    <div
      ...
      {/* Font loader */}
      <GoogleFontLoader href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&family=Poppins:wght@400;500;600;700;800&display=swap" arabic={locale === 'ar'} />
