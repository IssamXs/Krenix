...
export default function CarLanding({ landingPage, store }: Props) {
  const [openSection, setOpenSection] = useState<number | null>(0)
  const [showModal, setShowModal] = useState(false)

  const theme = store.theme?.config
  const bg = theme?.colors.background ?? CAR_TOKENS.bg
  const card = theme?.colors.card ?? CAR_TOKENS.card
  const primary = theme?.colors.primary ?? CAR_TOKENS.primary
  const text = theme?.colors.text ?? CAR_TOKENS.text
  const textMuted = theme?.colors.textMuted ?? CAR_TOKENS.textMuted
  const border = theme?.colors.border ?? CAR_TOKENS.border
  ...
  const headingFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.heading ?? CAR_TOKENS.heading}', sans-serif`
  const bodyFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.body ?? CAR_TOKENS.body}', sans-serif`
