...
export default function CarStoreHome({ store, products, landingPages = [], landingByProduct = {} }: {
  ...
  const headingName = sanitizeFontName(cfg?.fonts?.heading ?? CAR_TOKENS.heading)
  const bodyName = sanitizeFontName(cfg?.fonts?.body ?? CAR_TOKENS.body)
      ...
      <header className="sticky top-0 z-10" style={{ background: c.bg, borderBottom: `1px solid ${c.border}` }}>
        <div className="max-w-6xl mx-auto px-5 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            {store.logo_url
              ? <Image src={store.logo_url} alt={store.name} width={36} height={36} className="rounded object-contain" />
        ...
        <div className="max-w-6xl mx-auto px-5 py-12 grid md:grid-cols-4 gap-8">
          <div className="md:col-span-2">
            <div className="flex items-center gap-2.5 mb-3">
              {store.logo_url
                ? <Image src={store.logo_url} alt={store.name} width={32} height={32} className="rounded object-contain" />
