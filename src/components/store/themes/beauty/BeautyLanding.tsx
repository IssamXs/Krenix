...
import { BEAUTY_TOKENS } from './beautyDefaults'
...
export default function BeautyLanding({ landingPage, store }: Props) {
  const [openSection, setOpenSection] = useState<number | null>(0)
  const [showModal, setShowModal] = useState(false)

  const theme = store.theme?.config
  const bg = theme?.colors.background ?? BEAUTY_TOKENS.bg
  const card = theme?.colors.card ?? BEAUTY_TOKENS.card
  const primary = theme?.colors.primary ?? BEAUTY_TOKENS.primary
  const text = theme?.colors.text ?? BEAUTY_TOKENS.text
  const textMuted = theme?.colors.textMuted ?? BEAUTY_TOKENS.textMuted
  const border = theme?.colors.border ?? BEAUTY_TOKENS.border
  ...
  const headingFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.heading ?? BEAUTY_TOKENS.heading}', serif`
  const bodyFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.body ?? BEAUTY_TOKENS.body}', sans-serif`
  ...
  return (
    <div
      ...
      {/* Font loader */}
      <GoogleFontLoader href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&family=Cormorant+Garamond:wght@400;500;600;700&family=Jost:wght@400;500;600;700&display=swap" arabic={locale === 'ar'} />
