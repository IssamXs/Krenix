...
export default function HomeLanding({ landingPage, store }: Props) {
  const [openSection, setOpenSection] = useState<number | null>(0)
  const [showModal, setShowModal] = useState(false)

  const theme = store.theme?.config
  const bg = theme?.colors.background ?? HOME_TOKENS.bg
  const card = theme?.colors.card ?? HOME_TOKENS.card
  const primary = theme?.colors.primary ?? HOME_TOKENS.primary
  const text = theme?.colors.text ?? HOME_TOKENS.text
  const textMuted = theme?.colors.textMuted ?? HOME_TOKENS.textMuted
  const border = theme?.colors.border ?? HOME_TOKENS.border
