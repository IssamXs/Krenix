...
export default function SportLanding({ landingPage, store }: Props) {
  const [openSection, setOpenSection] = useState<number | null>(0)
  const [showModal, setShowModal] = useState(false)

  const theme = store.theme?.config
  const bg = theme?.colors.background ?? SPORT_TOKENS.bg
  const card = theme?.colors.card ?? SPORT_TOKENS.card
  const primary = theme?.colors.primary ?? SPORT_TOKENS.primary
  const text = theme?.colors.text ?? SPORT_TOKENS.text
  const textMuted = theme?.colors.textMuted ?? SPORT_TOKENS.textMuted
  const border = theme?.colors.border ?? SPORT_TOKENS.border
  ...
  const headingFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.heading ?? SPORT_TOKENS.heading}', sans-serif`
  const bodyFont = isRTL ? "'Tajawal', 'Cairo', system-ui, sans-serif" : `'${theme?.fonts?.body ?? SPORT_TOKENS.body}', sans-serif`
  ...
  return (
    <div
      ...
      {/* Font loader */}
      <GoogleFontLoader href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&family=Barlow:wght@400;500;600;700&family=Barlow+Condensed:wght@500;600;700;800;900&display=swap" arabic={locale === 'ar'} />
